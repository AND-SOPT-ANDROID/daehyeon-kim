...
class UserRepositoryImpl @Inject constructor(
    ...
    override suspend fun registerUser(user: User): Result<Unit> = runCatching {
        ...
        if (response.isSuccessful) {
            response.body()?.result?.no ?: throw RegisterError.UnknownError("회원번호가 반환되지 않았습니다.")
        } else {
    ...
    override suspend fun login(login: Login): Result<Unit> = runCatching {
        val requestDto = LoginMapper.mapToRequestLoginDto(login)
        val response = userRemoteDataSource.login(requestDto)

        if (response.isSuccessful) {
            val token =
                response.body()?.result?.token ?: throw LoginError.UnknownError("토큰이 반환되지 않았습니다.")
    ...
    override suspend fun fetchUserHobby(): Result<String> = runCatching {
        ...
        if (response.isSuccessful) {
            response.body()?.result?.hobby ?: throw HobbyError.UnknownError("취미 정보가 반환되지 않았습니다.")
        } else {
            throw when (response.code()) {
